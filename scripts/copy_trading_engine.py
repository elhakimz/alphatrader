        ...
        while self.is_running:
            try:
                ...
                async with httpx.AsyncClient(timeout=10) as client:
                    for mid in mids:
                        try:
                            ...
                            gamma_url = f"https://gamma-api.polymarket.com/markets/{mid}"
                ...
                for trade in unresolved:
                    ...
                    # Resolution Logic:
                    # outcome is typically "0" (YES) or "1" (NO)
                    # user_side: "BUY" (YES/0), "SELL" (NO/1)
                    user_won = False
                    if str(outcome) == "0" and trade['executed_side'] == "BUY":
                        user_won = True
                    elif str(outcome) == "1" and trade['executed_side'] == "SELL":
                        user_won = True
                    ...
                    self.db.update_copy_trade_pnl(trade['id'], pnl, status="RESOLVED")
                    print(f"[Copy] SETTLED {trade['id'][:8]}: {'WIN' if user_won else 'LOSS'} (${pnl:.2f})")

                    # Notify frontend
                    if self.on_action:
                        asyncio.create_task(self.on_action({
                            "type": "copy_settled",
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                for pos in data:
                    ...
                    trade = {
                        ...
                        "side": "BUY" if pos.get('outcomeIndex') == 0 else "SELL", 
                        "price": price,
                        "size_usdc": float(pos.get('currentValue') or pos.get('initialValue') or 0),
    ...
    async def _evaluate_copy(self, source_trade: dict):
        ...
        for config in configs:
            ...
            with self.db._get_conn() as conn:
                ...
                if config.get('daily_loss_limit_usdc') and daily_pnl <= -float(config['daily_loss_limit_usdc']):
                    print(f"[Copy] SKIP: Daily loss limit reached for {config['id'][:8]} (${daily_pnl})")
                    continue
            ...
            if config['allocation_mode'] == 'proportional':
                ...
                copy_size = (source_trade['size_usdc'] * config.get('proportional_bps', 0)) / 10000.0
            ...
            with self.db._get_conn() as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    record['id'], record['config_id'], record['session_id'], record['source_trade_id'],
                    record['paper_mode'], record['executed_side'], record['executed_price'],
                    record['executed_size_usdc'], record['status'], record['pnl'], record['created_at']
                ))
            ...
            print(f"[Copy] EXECUTED: Mirroring {source_trade['source_wallet'][:8]} -> {copy_size} USDC on {source_trade['market_id'][:8]}")
