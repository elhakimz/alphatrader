...
async def notify_main_app(alert):
    ...
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.post("http://localhost:8888/notify/new_alert", json=alert, timeout=5.0)
            if resp.status_code == 200:
                print(f"[Worker] Notification sent successfully for {alert['id']}")
            else:
                print(f"[Worker] Notification failed with status {resp.status_code}: {resp.text}")
    except Exception as e:
    ...
    while True:
        try:
            ...
            for m in normalized_markets:
                ...
                if opp:
                    db.insert_raw_inefficiency(
                        market_id=m["id"], type="ARBITRAGE",
                        gap=opp["details"]["gap"], yes=opp["details"]["yes"], no=opp["details"]["no"]
                    )
            ...
            for opp in cross_opps:
                db.insert_raw_inefficiency(
                    market_id=opp["market_id"], type="CROSS_MARKET",
                    gap=opp["edge_pct"] / 100.0, yes=opp["details"].get("p1", 0), no=opp["details"].get("p2", 0)
                )
                alert = alert_engine.process_opportunity(
                    {"id": opp["market_id"], "question": opp["market_question"]}, opp
                )
                if alert:
                    print(f"[Worker] ALERT: Cross-Market logic in {opp['market_question']}")
                    await notify_main_app(alert)

            # 3. AI Edge (Top 10 by volume)
            top_ms = sorted(normalized_markets, key=lambda x: x.get('volume', 0), reverse=True)[:10]
            for m in top_ms:
                ...
                if opp:
                    db.insert_ai_estimate(
                        ...
                        fair_prob=opp["details"]["fair_prob"], market_price=opp["details"]["market_price"],
                        confidence=opp["details"]["confidence"], reasoning=opp["details"]["reasoning"]
                    )
                    alert = alert_engine.process_opportunity(m, opp)
                    if alert:
